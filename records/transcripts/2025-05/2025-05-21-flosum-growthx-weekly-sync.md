...
participants: Girish Jashnani, Brian Evans, Beau Beamon, Saakshi Jain, Rosalia Parra, Daniel Lopes, Team GrowthX, Jakub Rudnik, Drew Noel, Kyle Gaudreau, Matthew Panzarino, Matt Lyman, Aida Knežević
...
Weekly sync between Flosum and GrowthX to discuss content strategy, performance reporting, and next steps for blog content creation and optimization.
...
- GrowthX set up a Looker Studio report for tracking blog performance, including GSC and GA4 data
- Flosum's Webflow CMS supports custom CTAs and banners, offering opportunities for content optimization
- GrowthX is working on no-index page cleanup and will provide updates in the coming weeks
- Flosum team to review and provide feedback on calibration articles by end of week
...
- Matt to ensure calibration articles are reviewed and feedback provided by end of week
- Beau to adjust Webflow permissions for GrowthX team
- GrowthX to continue work on no-index page cleanup, with updates expected next week
- Team to explore implementation of custom CTAs and banners in blog posts
- Consider creating a comprehensive "best data backup and archive tools" comparison article
...
**Aida Knežević:** And then the last page of the report is LLM referral traffic, so this page basically has a filter on it that it only includes referral traffic from LLM, such as, like, Chachi.

**Aida Knežević:** Gemini, Perplexity, Deepseek.

**Aida Knežević:** So you get like a good, typically we don't filter this page for our blog posts, as a lot of clients just want to see the impact on their entire site from LLMs.

**Aida Knežević:** So this is where you're going to be able to see that.

**Aida Knežević:** This pie chart right here, it's incorrect right now.

**Aida Knežević:** I'm going to have to fix it later because it should be showing Chachi PT, Gemini.

**Aida Knežević:** It should show like a breakdown between different LLMs, but I'm going to tackle this later because I really couldn't figure it out in time.
...
**Matt Lyman:** Quick question on the query strings in there for the search from the search console.

**Matt Lyman:** I think it's on the weekly performance.

**Matt Lyman:** It's GA4 potentially because it was the.
...
**Matt Lyman:** Just based off of how I'm looking at this right now, you would you would said that it's the query.
...
**Matt Lyman:** But it looks like if we scroll down to the next section of this, that's the query.

**Matt Lyman:** Yeah.

**Matt Lyman:** of right now, we kill it when people search for our name.
...
**Matt Lyman:** Matt, just to quickly interrupt.

**Beau Beamon:** You guys don't need design access to Webflow, do you?

**Beau Beamon:** Um, I mean, I'm not really sure.

**Aida Knežević:** So I don't believe I'm seeing Jacob shake his head.
...
**Jakub Rudnik:** I believe it's a no from my Webflow experience.

**Jakub Rudnik:** Thank you.

**Jakub Rudnik:** you.

**Jakub Rudnik:** Sorry to interrupt.
...
**Jakub Rudnik:** And then if we get, you know, 612 beyond, we'll start looking quarters, obviously, but with like the amount of traffic and then how this should be growing, I think month over month will make the most sense.

**Jakub Rudnik:** Just we'll zoom forward a little bit.

**Jakub Rudnik:** But we could even, you know, I don't think we've done this yet, but we could even just duplicate that view and then do a month over month view, too, so we can see it both just like the raw impressions and clicks and also a monthly month over month.

**Jakub Rudnik:** So that could be useful.

**Jakub Rudnik:** And then the only other addition is, I know we're starting, obviously, with new content.

**Jakub Rudnik:** I think that's the.

**Jakub Rudnik:** That should be the bulk as we've kind of discussed, but as we start to do refreshes too, we'll want to separate those and we've got a couple different ways to report on those, but since they will have some impressions and some traffic existing, know, we'll still want to separate that because like the moment we refresh, we haven't necessarily changed anything on their metrics.

**Jakub Rudnik:** We want to see that over time, but we'll, so we'll take a look at that separately.

**Jakub Rudnik:** There's a little bit different reporting, but just wanted to call that out that this is specific to the new content.
...
**Jakub Rudnik:** Only one other item to you, so apologies to go off script, but the no index pages, kind of that cleanup, that has not been forgotten.

**Jakub Rudnik:** I started it.

**Jakub Rudnik:** I just need to marry in some of like the data from real pages so I can make more recommendations on where to go.

**Jakub Rudnik:** Sometimes it's very simple, like we should just redirect here.

**Jakub Rudnik:** Other times getting a larger scope of all of your pages is more difficult.

**Jakub Rudnik:** So I'm still in that process.

**Jakub Rudnik:** So I'll have more next week.

**Jakub Rudnik:** I don't know with the number of pages.

**Jakub Rudnik:** The is about off.

**Jakub Rudnik:** I'll be able to get through that entire list within the next week, but have more that we can start to action and start to whittle that down.

**Jakub Rudnik:** Probably be a couple week project would be my guess, but thanks for the patience on that as I was coming back from vacation, just didn't get to it as much as maybe I would have been another week.

**Jakub Rudnik:** So keep you updated on that one.

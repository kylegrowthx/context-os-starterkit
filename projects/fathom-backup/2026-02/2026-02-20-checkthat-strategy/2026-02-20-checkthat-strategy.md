...
participants: Fred Fireflies (Fireflies.ai), Daniel Lopes (GrowthX Labs), Marcel Santilli (GrowthX), Stevie Kim (GrowthX)
...
CheckThat is pivoting from a self-serve dashboard to a high-value reporting service, validated by manual analysis work for client Eon that revealed actionable competitive and content insights (80% of citations are listicles). The self-serve funnel's near-zero activation (200 sign-ups/week, ~0 conversions) accelerates the pivot by eliminating legacy user support obligations. The immediate bottleneck is database infrastructure: Postgres cannot support the complex on-the-fly analysis required for prescriptive reports, forcing a TimescaleDB prototype while awaiting ClickHouse learnings from the OS project.

---

## Context

CheckThat has operated as a self-serve platform but discovered a more defensible and valuable business model through hands-on client work. When Marcel performed a manual analysis for Eon (a data storage company) using CheckThat data, agent-assisted research, and SEMrush augmentation, it produced actionable competitive intelligence that matched the client's actual needs: specific insights about source composition (49% analyst/direct competitors, 80% listicle content) that the current UI cannot deliver. This success proved that clients will pay for prescriptive reporting and strategic analysis rather than raw data access.

Simultaneously, the self-serve funnel metrics validate the pivot's timing. The product attracts 200 sign-ups per week with above-average sign-up-to-trial conversion (at or above industry standard), but activation drops to near-zero (only 1 user invited someone to a workspace; 1 user managed prompts in bulk). This signals that prospecting works, but the self-serve model cannot deliver value to unqualified users. Rather than burden the team with scaling a broken product, the zero-revenue state becomes an asset: the team can pivot to reporting without retraining or recompensating paying customers.

The engineering constraint is urgent: the current Postgres setup cannot handle the filtering, aggregation, and real-time slicing required for prescriptive reports. Jose is prototyping a TimescaleDB migration (a simpler, Postgres-compatible alternative) while the OS team evaluates ClickHouse in parallel. Both teams expect to converge on ClickHouse as the long-term solution, but the immediate goal is unblocking advanced filtering in the Overview dashboard. This database work is blocking all other feature development.

---

## Relevance

- **Strategic Planning**: Validates the product-market fit of the reporting model over self-serve, with clear evidence of client demand and pricing validation ($100/report offered by dinner attendees).
- **Engineering Priorities**: Database migration is the hard blocker for the entire product vision; all feature development is paused until TimescaleDB or ClickHouse is operational.
- **Sales & Positioning**: The Eon analysis is already being used in sales conversations and as a case study for how clients can use CheckThat; monthly AI visibility reports can become a thought leadership and lead generation vehicle.
- **Team Alignment**: Pedro (engineering lead) needs to be brought up to speed on the strategic pivot and why the team is focusing on data management automation rather than new features or sales-driven prompts.
...
**Stevie Kim (GrowthX)**
- Align Pedro on new strategy; bring him up to speed on pivot to reporting, why we're pausing feature work
- Read 3 guides in handbook (methodology, product, reporting vision); block 2h; send feedback to Marcel

**Marcel Santilli (GrowthX)**
- Schedule weekly 1h shaping session w/ Daniel Lopes to validate product direction and get pushback on vision
- Fix VZero preview demo; send to Daniel Lopes for feedback
- Schedule recorded walkthrough w/ Mario (engineering); send recording to Daniel for context on technical approach
- Send prescriptive directions to Stevie Kim on citation taxonomy/tagging system and backtag citations using new taxonomy

---

## Transcript

*Cleaned transcript capturing the strategic discussion on CheckThat funnel performance, engineering constraints, and product strategy validation.*

**Daniel Lopes:** How do you want to do this meeting? We could just go through Stevie's updates first if you can walk us through what's happening, and maybe we can talk about the CheckThat work you did this week, Marcel.

**Stevie Kim:** Yeah, so basically, sign-ups have been good and steady at about 200 people per week. But it's the further into the funnel where we're getting a lot of drop-offs. We had one person invite someone to the workspace—that's new. One person managed prompts in bulk. We're just not getting a lot of usage in general. People are really stoked on the public pages though—that's getting a lot of traffic.

**Daniel Lopes:** I think the sign-up conversion from landing page to trial is actually above industry standard. So that part's working. But yeah, activation is really where the issue is.

**Stevie Kim:** That's the whole shift in strategy though. Once we automate those reports and have them out for people, they'll understand the value.

**Marcel Santilli:** I have some good calls on CheckThat going. What I've been doing is taking all the data and manually exporting it from the app, feeding it to an agent with SEMrush data and screenshots, and having the agent draw conclusions. For a client like Eon, this was incredibly enlightening.

**Daniel Lopes:** Yeah, so just to be clear on the strategy: right now we have high sign-up volume but zero paying customers. That's actually a benefit for us because we can pivot to a reporting model without having to retrain or compensate legacy self-serve users. If people were already paying and locked into annual contracts, this transition would be painful.

**Stevie Kim:** Right. And we're focused on the right things—growth tickets, admin automation, research workflows. But we don't have the headcount because Pedro's been out and Jose is heads-down on the database migration.

**Daniel Lopes:** So on the database piece: Postgres isn't scaling for the analytical queries we need. We're doing a Timescale prototype since it's a one-to-one switch from Postgres, while Jakob and the OS team evaluate ClickHouse. Once ClickHouse works on the OS side, we'll import those learnings to CheckThat. The hard blocker is that we can't slice and dice data the way we need to without this migration.

**Marcel Santilli:** The database is definitely the constraint. But I want to zoom out: I think we should have at least an hour a week of shaping time—not backlog grooming, but actual vision validation. I need you, Daniel, to tell me if I'm smoking something or if the direction is right.

**Daniel Lopes:** Yeah, I wanted to do that yesterday but the OS just ate all my time. Let's do it now though.

**Marcel Santilli:** Okay, so let me walk you through what I did for the Eon analysis. I took their CheckThat data, augmented it with SEMrush research, and ran agent analysis on top. The key finding: 49% of their citations come from analysts or direct competitors—that's the heaviest concentration I've ever seen. And 80% of citations come from listicle or "best of" content.

**Daniel Lopes:** That's interesting because it's biased by the prompts we're tracking. But it's still actionable.

**Marcel Santilli:** Exactly. She had just signed a six-month contract with an SEO agency and was annoyed, but when I showed her this analysis, she immediately saw the value. She understood her competitive position and the content strategy opportunity. That's the report people will pay for—not a dashboard showing a bunch of raw numbers, but prescriptive competitive intelligence and content recommendations.

**Daniel Lopes:** And the monetization angle is clear too. If people at a dinner are offering to pay $100 for a report, that's real demand.

**Stevie Kim:** So what do we need to do to automate this?

**Marcel Santilli:** The blocking issue is the same as Daniel mentioned: we need better filtering and data aggregation. Right now I can only manually export. Once we can query and filter on-the-fly at scale, the automation is straightforward. And that's the database migration.

**Daniel Lopes:** We also need to get Pedro up to speed on this vision so he understands why we're pausing feature work and focusing on data management infrastructure instead.

**Marcel Santilli:** Agreed. I'll send directions on the taxonomy and tagging system we need for sources and citations, so the team can start automating that piece while we work through the database.

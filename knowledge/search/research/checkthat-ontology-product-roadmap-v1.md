...
│                                                          │
│  Visibility · Reputation · Alignment · Influence · Lift  │
└─────────────────────────────────────────────────────────┘
```
...
| Brand awareness % | Visibility Rate |
| Brand favorability | Reputation Score |
| Message accuracy | Alignment Score (THE DIFFERENTIATOR) |
| Earned media influence | Influence Score (internal vs external) |
| NPS | AI Endorsement |
| Brand health metric | AI Brand Health |

The killer insight: "No other AEO tool has the concept of alignment against a brand source of truth. They just show you what AI says. We tell you if AI is *right*." And with Influence, we go further: we tell you *why* AI thinks what it thinks, and where you have leverage to change it.
...
**Workflows power everything.** The Output SDK integration gives us durable, observable AI workflows for every automated process: `brand_context_generator`, `company_researcher`, `personas_generator`, `buyer_questions_generator`, `branded_classifier`, citation scraping, content brief generation, and more. Adding new analysis workflows (alignment scoring, influence analysis, reputation extraction) plugs into an architecture that already handles retries, tracing, and monitoring.
...
**What we're doing now is upgrading the intelligence.** The foundation captures what AI says. The next phase measures whether AI is *right*, identifies *why* AI thinks what it thinks, and tells brands *what to do about it*.
...
LAYER 3: MEASUREMENT           Visibility, Reputation, Alignment, Influence, Lift.
...
| 6. Measurement Purpose (WHY) | What this prompt is designed to reveal | Visibility, Reputation, Alignment, Influence |
...
### Layer 3: Measurement — Five Core Metrics

**What it is:** The intelligence extracted from AI responses. Not "did AI mention you?" but "does AI understand you correctly?" and "what's shaping its perception?" This is where CheckThat's value lives.

**Why it matters:** Five core metrics that cover everything the industry tracks, plus two unique dimensions (Alignment and Influence) that no competitor offers. Three composites that summarize it for leadership. The full spec is in the [Metrics Framework](checkthat-metrics-framework-v1.md).
...
```
TIER 1: CORE METRICS (five dimensions)
...
The first three are *output metrics* — what AI says. Influence is the *input metric* — what's driving what AI says. Lift is the *change metric* — whether it's moving in the right direction. Together they form a complete diagnostic: what, why, and whether.

**Visibility — "Does AI mention us?"**

Enhance existing visibility tracking with: Visibility Rate (split by unaided vs aided), Position (1st through afterthought), Stability Index (consistency over time), Cross-Engine Coverage (which engines know you). Builds mostly on existing `ProbeMention` data.

**Reputation — "How does AI describe us?"**

Go beyond polarity to: Sentiment Score (-100 to +100), Attribute Map (what concepts AI associates with you), Narrative Frame (what story AI tells — market leader, niche player, budget option), Recommendation Strength (5-level scale from strong endorsement to caveated), Comparative Reputation (how AI frames you vs each competitor). Requires new LLM-as-judge post-probe analysis workflow.

**Alignment — "Does AI match our truth?" (THE DIFFERENTIATOR)**

The killer feature. Compare AI's description against the brand context answer key. Score on 6 dimensions: Feature Accuracy, Pricing Accuracy, Positioning Match, Differentiator Recognition, Narrative Alignment, Factual Accuracy. Each scored 1-5 by LLM-as-judge. Hard dependency on Layer 1 — without the answer key, alignment is impossible.

**Influence — "What shapes AI's perception of us?"**

The diagnostic metric. Split into Internal Influence (your domain's weight) and External Influence (third-party sources). Internal: Own-Domain Citation Rate, Own-Domain Citation Share, Source Authority Rank, Content Responsiveness (v2). External: Third-Party Source Map, External Source Accuracy, External Source Sentiment, Signal Concentration.

Influence answers the question the other metrics can't: "My alignment score is 45% — is that because MY content is wrong, or because a third-party source is wrong?" Different diagnosis, different remedy.

| Situation | Diagnosis | Action |
|-----------|----------|--------|
| Low Alignment + High Internal Influence | Your own content is misleading AI | Fix your content |
| Low Alignment + High External Influence | Third parties are misleading AI | Fix external sources (G2 profile, Reddit, press) |
| Low Alignment + Low Influence | AI is hallucinating | Establish source authority from scratch |
| High Visibility + Low Internal Influence | Mentioned but your content isn't used | Technical AEO — schema, crawlability, content structure |

**Lift — "Is it getting better?"**

Track all metrics over time: Temporal Trend (directional movement), Competitive Shift (your change vs competitors), Cross-Engine Spread (expanding or contracting coverage). Content Lift and Authority Lift are v2 features requiring event tracking.

#### The Three Composites

```
TIER 2: COMPOSITES (three headline numbers)
  AI Brand Health       Vis + Rep + Align + Inf (equal weight, configurable)
  AI Share of Voice     Visibility share + Citation share + Position-weighted share
  AI Endorsement        Recommendation Strength + Position + Narrative Frame + Comparative Rep
```

**AI Brand Health** — the NPS of AI visibility. One number (0-100) that summarizes overall perception health. Includes Influence because a brand with great scores but no control over its narrative is fragile.

**AI Share of Voice** — competitive metric. Your presence vs competitors across the same prompts, combining mention share, citation share, and position-weighted share.

**AI Endorsement** — how strongly AI advocates for you when buyers ask. The NPS analog. Combines recommendation strength, position, narrative frame, and comparative reputation.

#### Key decisions

- Alignment scoring accuracy: target 80% agreement with human evaluators
- LLM cost: estimate $0.01-0.05 per response analysis. Batch efficiently, cache aggressively.
- Composite score weighting: equal as default (25/25/25/25 for Brand Health), configurable per workspace
- Engine weighting: market-share weighted as default with equal-weight toggle
...
**Why it matters:** Measurement without visualization is just a database. The views are how users develop intuition about their AI visibility, spot problems, and decide what to do. Users should see their AI Brand Health front and center.
...
Replace the current probe report + visibility chart + perception donuts with a single headline: AI Brand Health. Big number (0-100), trend arrow, sparkline. Below it: Visibility, Reputation, Alignment, and Influence as cards with individual scores and trends. Competitive comparison against top 3 competitors on the same prompts.

One glance answers: "How is my AI brand perception today, and am I getting better or worse?"

#### Visibility view

A dedicated drill-down for understanding where and how your brand shows up. Unaided visibility rate by buyer stage (heatmap), position distribution (chart), cross-engine coverage map (per-engine breakdown), stability index over time, top prompts by visibility.

#### Reputation view

Beyond positive/negative. Polarity breakdown (enhanced existing donut), attribute map (tag cloud of concepts AI associates with you), recommendation strength (gauge), narrative frame (what story AI tells), comparative reputation (side-by-side vs top competitor).
...
The diagnostic drill-down. Internal vs External influence split. Own-domain citation rate and share. Source authority rank in category. Third-party source map showing which external domains shape AI's perception. External source accuracy and sentiment. Signal concentration analysis. "Your alignment is 45% because 72% of AI's info about you comes from an outdated G2 review" — that's what this view surfaces.
...
| 3. "Here's how AI sees you" | THE WOW MOMENT. Pull shared index data. Show visibility rate, competitor comparison, misalignment flags, engine breakdown. Data they've never seen. | 30s |
...
- Their visibility rate — "AI mentions [brand] in X% of relevant buyer questions"
...
- Influence snapshot — "AI's perception of you is primarily shaped by [G2, your website, Reddit]"
...
**Why it matters:** Without intelligence, CheckThat is a dashboard people look at and wonder "is this good?" With intelligence, it's an advisor that says "your pricing alignment dropped 20% this week — AI is citing your old pricing on 3 of 5 engines — the source is an outdated G2 review — update your G2 profile and add schema markup to your pricing page."

The methodology docs define a full Gap-to-Content Loop: identify where you're invisible or misrepresented → create content to fill the gap → measure lift → repeat. Layer 6 automates the "identify" and "recommend" steps of that loop. Influence data makes the recommendations specific: not just "your alignment is low" but "your alignment is low because [specific source] says [specific wrong thing]."
...
**1. Score summary**
- AI Brand Health this week vs last week (trend + delta)
- Visibility, Reputation, Alignment, Influence sub-scores with directional change
...
- Visibility changes — "You appeared in 5 new prompts this week" or "You disappeared from 3 prompts"
- Reputation shifts — "Your narrative frame shifted from 'niche player' to 'emerging leader' on Perplexity"
- Alignment gaps — "AI is still citing your old pricing in 40% of pricing-related prompts"
- Influence shifts — "A new TechCrunch article is now 15% of external citations about you — and it's accurate"
- Competitive movements — "Competitor X gained 3 positions in Direct Comparison prompts"

**3. Action recommendations**
Map each significant change to the Gap-to-Content Loop, enriched by Influence data:

| What happened | Influence diagnosis | What to do |
|---------------|-------------------|-----------|
| Not appearing in "[X] vs [Y]" | Low internal influence on comparison prompts | Create a definitive comparison page, optimize for AI citation |
| Incorrect pricing in AI | G2 review from 2024 is 40% of external citations | Update G2 profile first, then update your pricing page with schema markup |
| Not appearing for "best [category] for [use case]" | No source authority in this topic cluster | Create industry-specific landing pages and case studies |
| Negative sentiment on specific attribute | Reddit thread is 30% of external citations | Respond on Reddit, publish counter-evidence on your blog |
| Strong early-stage but disappearing in evaluation | Competitor blog dominates evaluation citations | Build deeper evaluation-stage content (comparisons, case studies, ROI) |
| Mentioned but never cited | High visibility, zero internal influence | Technical AEO — improve content structure, add schema, lead with answers |

Recommendations aren't AI-generated creative strategies. They're pattern-matched from the methodology's playbook, enriched with specific Influence data. "Your recall dropped in healthcare best-of prompts + 72% of AI's info comes from an outdated G2 review" = actionable.
...
The ontology's equivalent of a traditional brand tracking wave. Monthly or bi-weekly snapshots that capture the full state: all five core metrics, all sub-metrics, all competitors, all engines. These become the baseline for measuring lift. "Before we published the comparison page, visibility for Direct Comparison prompts was 35%. After: 52%."

**2. Competitive Benchmarks**
Same prompts, all competitors tracked. AI Share of Voice, visibility rate, reputation, alignment — all compared head-to-head. This is the competitive benchmarking report that marketing teams present to leadership. "Here's where we stand vs the field, and here's what's changed since last month."
...
- **Visibility:** flag changes >10% week-over-week on Tier 1 prompts
- **Reputation:** flag polarity shifts (positive→negative or vice versa) on branded prompts
- **Alignment:** flag any new factual inaccuracy detected (pricing wrong, feature missing)
- **Influence:** flag when a new external source enters the top 3 (new source shaping your narrative)
...
| **Cycle 3: Measurement** | Weeks 5-7 | **Layer 3** (Measurement — all five core metrics + composite scores) + **Layer 4** (Views — primary score view, metric drill-downs including Influence view) |
| **Cycle 4: Action** | Weeks 8-9 | **Layer 4** (Views — coverage view, engine view) + **Layer 6** (Intelligence — weekly reports with Influence-enriched recommendations, snapshots, action recommendations) + **Layer 5** (Onboarding — wow moment with real alignment + influence data) |
...
| Bet 4: Onboarding v2 | **Layer 5** Onboarding | Same vision. Specific data requirements for wow moment. Fallback strategy for unknown brands. Influence snapshot in onboarding. |
| Bet 5: Metrics & Dashboard | **Layer 3** Measurement + **Layer 4** Views | Split into engine and experience. Five core metrics (adding Influence as new dimension). Three refined composites. Influence view is new. |
| Bet 6: Reports & Insights | **Layer 6** Intelligence | Elevated from "dashboard feature" to its own layer. Weekly reports enriched with Influence diagnosis. Phased delivery (dashboard → email → Slack). |
...
3. **Five core metrics instead of four.** Influence is new — it measures what shapes AI's perception (internal vs external) and tells brands where they have leverage. This transforms "your score is low" into "your score is low because [specific source] — fix it here."
4. **Refined metric names.** Visibility (not Recall — speaks AEO language), Reputation (not Sentiment — richer than polarity), Influence (not Citation — covers the full input landscape). Alignment and Lift stay.
5. **Three new layers.** Views (Layer 4), Onboarding (Layer 5), and Intelligence (Layer 6) separate the *engine* from the *experience* and add the "so what?" layer.
6. **Intelligence enriched by Influence.** Weekly reports don't just say "your alignment dropped" — they say "your alignment dropped because [specific external source] is saying [specific wrong thing]." Actionable diagnosis.
7. **Full metrics framework companion doc.** Every metric defined, computed, and mapped to the industry. Covers every competitor metric plus two unique dimensions.
...
**Question:** How to weight Visibility, Reputation, Alignment, and Influence in AI Brand Health?

**Recommendation:** Equal weighting (25/25/25/25) as default. Per-workspace override capability. Show what the score would be under different weightings in settings. Including Influence in the composite means brands with great scores but no control get a lower health score — this is intentional.
...
### 9. Influence analysis depth

**Question:** How deep does external source analysis go at launch?

**Recommendation:** v1 surfaces the Third-Party Source Map (which domains are cited) and Own-Domain Citation Share (your domain's weight). External Source Accuracy (are third parties right about you?) requires cross-referencing third-party content against brand context, which is an LLM-heavy operation. Ship Source Map and Citation Share first. Add accuracy analysis as a v1.5 enhancement.

---

*This roadmap turns the Prompt Taxonomy & Ontology from a strategy document into a build plan. Six layers that take us from "a visibility dashboard that shows mentions" to "an AI brand research platform that tells you if AI is right about you, why it thinks what it thinks, and what to do when it's wrong."*

*The foundation is strong. The engine runs. The data exists. Now we add the intelligence.*

*Next steps: Review at team meeting. Resolve the 9 open decisions. Assign layer owners. Start Cycle 1.*
